 ...
 */
/* G&W: 16 KiB colour LUT lives in DTCM (allocated in graphics_init). */
int *totalpalette = NULL;
...
{
    /* Hot RGB565 LUT (~16 KiB) in DTCM; ITCM is reserved for .text. */
    if (totalpalette == NULL)
        totalpalette = (int *)dtc_malloc(16 * 16 * 16 * sizeof(int));
